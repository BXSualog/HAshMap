...
export default function Onboarding({ onComplete }: OnboardingProps) {
  const { isLargeScreen, layout, spacing, width, height } = useResponsive();
  ...
  const skipToDone = () => {
    onComplete(); // Skip directly completes the tutorial
  };
  ...
  const renderIcon = () => {
    if (currentSlide.image) {
      // Dynamic size for images on tablets
      const baseDim = isLargeScreen ? 480 : 300;
      let scaleStyles: any = { width: baseDim, height: baseDim };
      
      if (currentSlide.id === 3) {
        scaleStyles = { width: baseDim * 1.2, height: baseDim * 1.2, marginTop: 40, marginBottom: -15 };
      } else if (currentSlide.id === 4 || currentSlide.id === 5) {
        scaleStyles = { width: baseDim * 1.2, height: baseDim * 1.2, marginTop: 40, marginBottom: -25 };
      }
    ...
    const IconComponent: any = currentSlide.iconLibrary === 'MaterialCommunityIcons' ? MaterialCommunityIcons : Ionicons;
    return (
      <View style={[styles.iconContainer, isLargeScreen && { width: 320, height: 320, borderRadius: 160 }]}>
        <IconComponent name={currentSlide.icon} size={isLargeScreen ? 200 : 144} color="#3b82f6" />
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Slide Content */}
      <Animated.View style={[styles.contentContainer, { opacity: fadeAnim, maxWidth: isLargeScreen ? 800 : '100%', alignSelf: 'center' }]}>
        {renderIcon()}
        <Text style={[
          styles.title,
          { fontSize: getResponsiveFontSize(currentSlide.id === 1 ? '3xl' : '2xl', isLargeScreen) },
        ]}>{currentSlide.title}</Text>
        <Text style={[
          styles.subtitle, 
          { fontSize: getResponsiveFontSize('base', isLargeScreen), lineHeight: isLargeScreen ? 32 : 24 }
        ]}>{currentSlide.subtitle}</Text>
      </Animated.View>
